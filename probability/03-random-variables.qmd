...
$$\boxed{F(x) = \sum_{x_i \leq x} P(X = x_i) \text{ and } f(x_j) = P(X = x_j)}$$

On top of that, the PDF is such that:

$$\boxed{0 \leq f(x_j) \leq 1 \text{ and } \sum_j f(x_j) = 1}$$
...
$$\boxed{F(x) = \int_{-\infty}^x f(y) dy \text{ and } f(x) = \frac{dF}{dx}}$$

On top of that, the PDF is such that:

$$\boxed{f(x) \geq 0 \text{ and } \int_{-\infty}^{+\infty} f(x) dx = 1}$$
