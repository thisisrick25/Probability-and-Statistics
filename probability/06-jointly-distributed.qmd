...
(D) $$\boxed{f_{XY}(x_i, y_j) = P(X = x_i \text{ and } Y = y_j)}$$ (C) $$\boxed{f_{XY}(x,y) \Delta x \Delta y = P(x \leq X \leq x + \Delta x \text{ and } y \leq Y \leq y + \Delta y)}$$

- Marginal density ― We define the marginal density for the variable $X$ as follows:

(D) $$\boxed{f_X(x_i) = \sum_j f_{XY}(x_i, y_j)}$$ and (C) $$\boxed{f_X(x) = \int_{-\infty}^{+\infty} f_{XY}(x,y) dy}$$

- Cumulative distribution ― We define cumulative distribution $F_{XY}$ as follows:

(D) $$\boxed{F_{XY}(x,y) = \sum_{x_i \leq x} \sum_{y_j \leq y} f_{XY}(x_i, y_j)}$$ and (C) $$\boxed{F_{XY}(x,y) = \int_{-\infty}^x \int_{-\infty}^y f_{XY}(x',y') dx' dy'}$$
...
(D) $$\boxed{E[X^p Y^q] = \sum_i \sum_j x_i^p y_j^q f(x_i, y_j)}$$ and (C) $$\boxed{E[X^p Y^q] = \int_{-\infty}^{+\infty} \int_{-\infty}^{+\infty} x^p y^q f(x,y) dy dx}$$
...
$$\boxed{\mathrm{Cov}(X,Y) \triangleq \sigma_{XY}^2 = E[(X - \mu_X)(Y - \mu_Y)] = E[XY] - \mu_X \mu_Y}$$
